...
export class FeedEngine {
  ...
  // knobs
  traffic = 48
  cacheHit = 0.85 // repurposed as "hot share": fraction of queries hitting the hot tier
  writeShare = 0.18 // repurposed as "query share": queries vs ingest
  running = true
  speed = 6
  celeb = false
  ...
  setStage(i: number) {
    ...
    if (this.nodes.idx) this.nodes.idx.instances = this.stage.idx ?? 2
    this.cardinality = 0
    this.clusters = 1
    this.quota = false
    this.celeb = false
  ...
  private slots(nd: SimNode): number {
    ...
    if (nd.kind === 'idx') s = nd.instances * nd.slotsPer * this.clusters
    return Math.max(1, s)
  }
  ...
  setIndexers(n: number) {
    if (this.nodes.idx) this.nodes.idx.instances = n
  }
  setCardinality(n: number) {
    this.cardinality = n
  }
  setClusters(n: number) {
    this.clusters = n
  }
  setQuota(b: boolean) {
    this.quota = b
  ...
  killNode() {
    ...
    const cand = ['idx', 'hot', 'buffer', 'gw'].find(
  ...
  private spawn(type: ReqType) {
    if (this.particles.length >= MAX_PARTICLES) {
      ...
      if (!this.quota) this.dropWin.push(this.now)
      return
    }

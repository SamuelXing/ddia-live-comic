...
export const season2: Chapter = {
  ...
  minutes: 9,
  caption:
    'Season 1 asked where data lives, and every answer assumed the data sits still and a query comes to visit it. This season turned that around, and the trade it argues about is not consistency against availability — it is **freshness against everything freshness costs.** Read the chapters straight through and they look like thirteen unrelated systems. Read the acts and there is a ladder: each one removes a different delay between something happening and somebody being able to see it, by a mechanism that has nothing to do with the others, and every single one sends the same bill. This page is that ladder, the ideas that cross it, and an ending the season declines to settle — because the field has not settled it either.',
  steps: [
    {
    ...
    },
    {
      ...
      span: 2,
      body: [
        'Four cells per act, the same ledger Season 1 keeps. **What forced it, what got sold, what that bought, and what it cost** — and this season’s cost column is worth reading on its own, because it is a list of the numbers that end up in somebody’s capacity plan. Resident memory. Window retention. Index size. History that only grows.',
    ...
    },
    {
      ...
      span: 2,
      body: [
        'The ideas that never got a chapter, because they are not any one paper’s idea. Deliberately no overlap with Season 1’s six, even where one of those genuinely resurfaces — the log turns up again, hard, in the epilogue, and it is already written up there rather than padded out again down here.',
      ],
    ...
    },
    {
      ...
      body: [
        ...
        'So the useful question is not which one is right. It is **which one describes what you already have.** If you are running four copies of one dataset because atomicity was missing from the layer underneath, that is waste, and Chapter 29 is about you. If you are running a search index and a columnar store because a full-text query and a scan of one column are genuinely different jobs, that is not waste, and no amount of merging will make it so — and then the thing worth fixing is the tangle of bespoke pipelines between them, which is Chapter 30. Most architectures have some of each, and the two chapters are better read as a diagnostic than as a choice.',
      ],
  ...
  finale: {
    ...
    body: 'Thirteen papers about one duration — how long after something happens can somebody see it — and four acts that each shorten it by a mechanism with nothing in common with the others. Keep the middle of the computation in memory and recover by replaying the recipe. Key results to when things happened rather than when they turned up, speak before the input is complete, and correct yourself afterwards. Treat the change as the unit of work so an answer is updated instead of recomputed. Make convergence a property of the data so a replica alone in a tunnel is correct rather than degraded. Four unrelated ideas, and every one of them is paid for in state that has to stay hot: memory that cannot spill, windows held open for arrivals that may never come, every difference kept and indexed, a change history nobody may truncate. That is the season, and it is the thing to carry out of it — when somebody offers you fresher data, the question is not whether the mechanism works, it is what has to stay resident for it to. And the ending is two people looking at the same heap of parts and disagreeing about whether the right move is to put the lid back on, which is not a hole in the argument. It is where the argument currently is.',
  },

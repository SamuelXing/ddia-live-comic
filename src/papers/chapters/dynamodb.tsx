...
export const dynamodb: Chapter = {
  ...
  caption:
    'Chapter 5 opened with a shopping cart that was not allowed to refuse a write, and Amazon paid an enormous price to keep that promise: no leader, sloppy quorums, and versions the application had to reconcile itself. Fifteen years later the same company published a paper about the system that carries the name, and it runs an elected leader per partition, refuses writes when the quorum is gone, and offers a current read to anyone willing to pay for one. **Nobody published a retraction.** But before you read this as a defeat, notice where the retreat actually started — and it was not in an argument about consistency. It was that Amazon’s own engineers stopped choosing Dynamo.',
  steps: [
    {
      n: 'Step 01',
    ...
    },
    {
      ...
      span: 2,
      body: [
        'This one is different from every other chapter in the book. You are not starting from a blank page — you are starting from a system that works, a paper the industry has been citing for a decade, and ten years of tickets. The famous retreat is the easy part. What the paper actually spends its length on is everything after it.',
      ],
      diagram: (
        <DesignIt
          spec={{
    ...
    },
    {
      ...
      body: [
        ...
        '**And this chapter is the season doing its own trick to itself.** Act I lost the single machine, Act II lost the master, Act III priced agreement, Act IV bought transactions back twice in different currencies, Act V promoted the log, Act VI split analytics off and then rented the machines to run it. Here the leaderless write goes, and what comes back is an operator nobody has to hire. *The season close is where that pattern gets laid out properly; this page is only the last instance of it.*',
        '**What dates is the answer, not the question.** Nothing in the 2007 paper was refuted here; the premises underneath it moved — the machines went from bought to rented, and the operator went from a colleague to a stranger — and the conclusions moved with them. A design is an argument about the hardware and the economics of one particular year, which is why the useful thing to take from any of these papers is the pressure rather than the mechanism. *Season 2 is about what happens when the data stops sitting still.*',
      ],
    },
  ],
  bubbles: [

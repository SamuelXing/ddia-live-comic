...
export const dbsp: Chapter = {
  ...
  dek: 'Two chapters have made the case for maintaining an answer instead of recomputing it, and in both of them a person had to work out by hand what the incremental version was. Forty years of that produced one paper per class of query. This one produces a procedure — and proves it in a theorem prover.',
  minutes: 16,
  paper: {
  ...
  steps: [
    ...
    },
    {
      ...
      span: 2,
      body: [
        'None of these moves is an engineering choice. They are decisions about what to *model* things as, which is the point: the previous two chapters made every engineering choice there was and still could not produce a procedure.',
      ],
      diagram: (
        <DesignIt
          spec={{
    ...
    },
    {
      ...
      body: [
        ...
        '*The honest row is the fourth one.* Aggregates like minimum and maximum are not linear and have no such trick, because taking away the smallest element means finding whatever was second, and a running minimum has no idea what that was. Done naively that is the whole database per update. The paper does not pretend otherwise; it points at a known sliding-window aggregation scheme that brings it to logarithmic, implementable as a custom operator — **which is the modularity claim being cashed**: a hand-optimised operator drops into the same framework and inherits the incrementalization theory rather than needing its own.',
      ],
      code: {
  ...
  next: { title: 'Who Writes the Merge Function', slug: 'crdt' },
}
